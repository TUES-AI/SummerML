window.SUMMERML_CHALLENGES = [
  {
    ...
    contract: { input: "N × 1 × 32 × 32", output: "N × 10 logits", objective: "Cross-entropy" },
    milestones: [
      "Sketch C1 → S2 → C3 → S4 → C5 → F6 and write every tensor shape.",
      "Build the tanh and average-pooling baseline before changing activations.",
      "Split only the training set; track validation accuracy and one loss curve.",
      "Try one change at a time, then save the exact checkpoint contract below.",
    ],
  ...
  },
  {
    ...
    contract: { input: "N × 1 × 500", output: "N × 2 logits", objective: "Cross-entropy" },
    milestones: [
      "Trace one 8 → 5 → 3 convolution block and its shortcut on a length-500 signal.",
      "Stack the 64 → 128 → 128 blocks and classify after global average pooling.",
      "Use a stratified training holdout; first reproduce the unaugmented baseline.",
      "Add small signal perturbations separately and keep only measured improvements.",
    ],
  ...
  },
  {
    ...
    contract: { input: "N × 24 features", output: "N binary logits", objective: "BCEWithLogitsLoss" },
    milestones: [
      "Keep 80% of each contiguous class, then make a stratified validation split.",
      "Train the paper-style dense baseline before adding normalization or dropout.",
      "Select checkpoints by validation ROC-AUC, not classification accuracy.",
      "Ablate width, depth, normalization, and dropout one variable at a time.",
    ],
  ...
  },
  {
    ...
    contract: { input: "N × 784 × 1", output: "N × 10 logits", objective: "Cross-entropy" },
    milestones: [
      "Reshape each image row-major to 784 scalar steps; never reorder the pixels.",
      "Classify only from the final LSTM output and verify the sequence dimensions.",
      "Start with a small batch, RMSProp, gradient clipping, and a training holdout.",
      "Tune hidden size or initialization only after the 128-unit baseline learns.",
    ],
  ...
  },
  {
    ...
    contract: { input: "N × 3 × 32 × 32", output: "N × 100 logits", objective: "Cross-entropy" },
    milestones: [
      "Write one mlpconv block: a spatial convolution followed by two 1×1 projections.",
      "End with 100 class maps and global average pooling—no large linear head.",
      "Train the no-augmentation paper baseline before adding crop and flip.",
      "Compare augmentation, width, and schedule in separate experiments.",
    ],
  ...
  },
  {
    ...
    contract: { input: "N × 3 × 64 × 64", output: "N × 200 logits", objective: "Cross-entropy" },
    milestones: [
      "Trace the scaled five-convolution path until it produces a 128 × 3 × 3 map.",
      "Add the two 1024-unit hidden layers, ReLU, LRN, pooling, and dropout.",
      "Establish a crop-and-flip baseline using only a training-set holdout.",
      "Test robustness transforms individually; keep validation labels out of training.",
    ],
  ...
  },
  {
    ...
    contract: { input: "N × 128 token IDs", output: "N × 2 logits", objective: "Cross-entropy" },
    milestones: [
      "Load exactly the named checkpoint and tokenizer with a fresh two-class head.",
      "Confirm every parameter has requires_grad=True before creating AdamW.",
      "Fine-tune on the training split and select epochs with a training-set holdout.",
      "Before saving, verify that the backbone—not only the classifier—changed.",
    ],
